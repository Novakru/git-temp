...
enum AggrOp
{
  ...
  AGGR_MAX,       ///< "max"
  AGGR_MIN,       ///< "min"
  AGGR_COUNT,     ///< "count not include null"
  AGGR_COUNT_ALL, ///< "count include null"  
  AGGR_NONE       ///< no aggregate
};
...
struct RelAttrSqlNode
{
  ...
  bool valid = true;               ///< valid                       判断字段的合法性
};
 ...
 * @ingroup SQLParser
 * @details 参与inner join的所有表，也就是 relations。
 * where 条件 conditions，这里表示使用AND串联起来多个条件。正常的SQL语句会有OR，NOT等，
 * 甚至可以包含复杂的表达式。
 */
